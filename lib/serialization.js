...
function serializeItem(item, _offset) {
  ...
  if (typeof item === 'string') {
    res = item;
    res = res.replace(/\\/g, '\\\\');   // Escape backslash
    res = res.replace(new RegExp(stringDelimiter, "g"), '\\' + stringDelimiter);   // Escape string delimiter

    //Object.keys(charactersToEscape).forEach(function (key) {
      //res = res.replace(new RegExp(key, "g"), charactersToEscape[key]);
    //});

    // Make escaped characters visible in the string so that eval understands them and we can change them
  ...
  if (item && item.constructor && item.constructor.name === 'Date') {
    return 'new Date("' + item.toString() + '")';
  }

  if (item && item.constructor && item.constructor.name === 'ObjectID') {
...
function serializeObjectKey(key) {
  if (key.match(/^[a-zA-Z][a-zA-Z0-9]*$/)) {
    ...
    return serializeItem(key, 0);   // Will use the "string" delimiter
  }
}
